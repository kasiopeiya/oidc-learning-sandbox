---
name: code-dev
description: 'アプリケーション実装の一気通貫オーケストレーションワークフロー。TDD実装→コードレビュー→レビュー指摘修正→CI実行→CI指摘修正→設計書整合性チェックを人間への確認なしに自律実行する。CLAUDE.md の開発フロー Step 6（frontend/backend実装）で使用。Issue番号を引数に指定すること（例: /code-dev 15）。CDKには使用しない。'
...
TDD実装 → コードレビュー → レビュー指摘修正 → CI実行 → CI指摘修正 → 設計書整合性チェック を順次自律実行する。
...
7. 各フェーズの出力を**そのまま全文表示**する（要約・加工・コメント追加は禁止）
