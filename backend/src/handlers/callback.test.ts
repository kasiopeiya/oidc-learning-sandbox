...
describe('callback handler', () => {
  ...
  describe('異常系', () => {
    ...
    it('トークン交換失敗時は/error?error=authentication_failedへリダイレクトする', async () => {
      ...
      // トークン交換失敗
      vi.mocked(client.authorizationCodeGrant).mockRejectedValue(new Error('Token exchange failed'))
    ...
    it('ネットワークエラー（TypeError）時は/error?error=network_errorへリダイレクトする', async () => {
      ...
      vi.mocked(client.authorizationCodeGrant).mockRejectedValue(new TypeError('fetch failed'))
