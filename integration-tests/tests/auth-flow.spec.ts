 ...
 * 認証フローE2Eテスト
 *
 * OIDC認可コードフローの完全なE2Eテストを実施する。
 * - テストユーザーの作成（Cognito Admin API）
 * - ログイン画面からの認証
 * - 口座作成APIの呼び出し
 * - 口座番号の表示確認
 * - テストユーザーの削除（クリーンアップ）
...
test.describe('AUTH-001: 認証フロー完全E2Eテスト', () => {
   ...
   * 1. トップページにアクセス
   * 2. 「口座作成」ボタンをクリック
   * 3. Cognitoホスト UIにリダイレクト
   * 4. メールアドレスとパスワードを入力してログイン
   * 5. /callback にリダイレクト
   * 6. 口座番号が表示されることを確認
   */
  test('ログインから口座番号表示までの完全フロー', async ({ page }) => {
    // ============================================================
    // Step 1: トップページにアクセス
    // ============================================================
...
test.describe('AUTH-002: 認証エラーテスト', () => {
  /**
   * 存在しないユーザーでログインを試みた場合のテスト
   */
  test('存在しないユーザーでログイン時にエラーが表示される', async ({ page }) => {
    // ============================================================
